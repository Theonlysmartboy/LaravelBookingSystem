...
class SettingsController extends Controller {
    ...
    public function editAccount(Request $request, $id = null) {
        if (Session::has('adminSession')) {
            if ($request->isMethod('post')) {
                $data = $request->all();
                Setting::where(['id' => $id])->update(['bank' => $data['product_name'], 'branch' => $data['product_code'], 'name' => $data['product_town'], 'account_no' => $data['product_town_code'], 'paybill_no' => $data['product_color']]);
